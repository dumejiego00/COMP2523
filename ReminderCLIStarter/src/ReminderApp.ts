...
export default class ReminderApp {
  ...
  private handleToggleCompletion(): void {
    ...
    if (this._remindersHandler.size() === 0) {
      ...
      const index = this.getUserChoice("reminder number to toggle", true);
      this._remindersHandler.toggleCompletion(Number(index));
      Logger.log("\n  🏁   Reminder Completion Toggled");
    }
  }
  ...
  private handleModifyReminders(): void {
    ...
    if (this._remindersHandler.size() === 0) {
      ...
      Logger.logReminders(this._remindersHandler.reminders);
      const index = this.getUserChoice("reminder number to edit", true);
      const newDescription = this.getUserChoice("Description", false);
      this._remindersHandler.modifyReminder(Number(index), newDescription);
      const toggle = ReminderApp.checkUserToggleChoice();
      if (toggle) {
        this._remindersHandler.toggleCompletion(Number(index));
      }
      Logger.log("\n  🏁   Reminder Modified");
  ...
  private handleAddReminder(): void {
    /*
            ...
            ( 4 ) Logger.log('\n  🏁  Reminder Added');
        */
    const reminder: string = this.getUserChoice("reminder", false);
    const tag: string = this.getUserChoice("tag", false);
    this._remindersHandler.addReminder(reminder, tag);
    Logger.log("\n  🏁  Reminder Added");
  }
  ...
  private handleSearchReminders(): void {
    ...
    */
    if (this._remindersHandler.size() === 0) {
      ...
      const resultArray = this._remindersHandler.search(keyword);
      Logger.logSearchResults(resultArray);
    }
  }
